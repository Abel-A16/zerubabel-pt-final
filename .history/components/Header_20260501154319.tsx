...
function Header({ socials }: Props) {
  ...
  // Helper function to ensure social URLs are valid
  const getSocialUrl = (url: string) => {
    ...
    // Handle common social platforms
    ...
    for (const [platform, template] of Object.entries(platformMap)) {
      if (lowerUrl.includes(platform) || lowerUrl === platform) {
        return template;
      }
    }
    return url;
  ...
  // Filter out invalid socials
  const validSocials =
    socials?.filter((social) => social?.url && social.url.trim() !== "") || [];
      ...
      {isMobileMenuOpen && (
        ...
        >
          <div className="flex flex-col p-6 gap-5">
            ...
            {/* Mobile Social Icons - Fetched from Sanity */}
            {validSocials.length > 0 && (
              <div
